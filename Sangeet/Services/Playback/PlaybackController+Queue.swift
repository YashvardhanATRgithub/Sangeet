...
extension PlaybackController {
    ...
    // MARK: - Legacy / UI Compatibility
    
    // UI expects separate user vs context queues.
    // For now, we map everything to "Context Queue" (the main queue).
    // TODO: Implement proper User Queue (Play Next/Later) separation.
    ...
    var contextQueue: [Track] {
        ...
        return Array(queue.suffix(from: currentQueueIndex + 1))
    }
    
    func playFromUserQueue(at index: Int) {
        // Not implemented
    }
    
    func playFromContextQueue(at index: Int) {
        let actualIndex = currentQueueIndex + 1 + index
        playTrackAtIndex(actualIndex)
    }
    
    // For UI compatibility with LibrarySongsView
    func startPlaylist(_ tracks: [Track], startingAt index: Int = 0) {
        playTracks(tracks, startingAt: index)
    }
}
