...
//  Sangeet
//
//  Created by Yashvardhan on 31/10/25.
//
...
class PlaybackController: ObservableObject {
    ...
    private func setupNotifications() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(refreshCurrentTrackMetadata),
            name: .libraryDidUpdate,
            object: nil
        )
    }
    
    @objc func refreshCurrentTrackMetadata() {
        guard let track = currentTrack, let dbId = track.trackId else { return }
        // Fetch fresh copy from database to update favorite status
        Task { @MainActor in
            if let freshTrack = await DatabaseManager.shared.getTrack(id: dbId) {
