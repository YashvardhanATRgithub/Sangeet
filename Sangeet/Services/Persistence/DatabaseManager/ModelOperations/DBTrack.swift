...
extension DatabaseManager {
    ...
    func getTrack(id: Int64) async -> Track? {
        try? await dbQueue.read { db in
            try Track.fetchOne(db, key: id)
        }
    }
    
    // Alias for compatibility with Legacy UI
    func fetchAllTracks() async throws -> [Track] {
        try await getAllTracks()
    }
    ...
    // Delete tracks in a folder (for LibrarySettingsView)
    func deleteTracks(in folderURL: URL) async {
        do {
            try await dbQueue.write { db in
                try Track.filter(Track.Columns.path.like("\(folderURL.path)%")).deleteAll(db)
            }
            Logger.info("Deleted tracks in folder: \(folderURL.path)")
        } catch {
            Logger.error("Failed to delete tracks in folder: \(error)")
        }
    }
    

    
    /// Delete a track by numeric ID
    func deleteTrack(trackId: Int64) async throws {
        try await dbQueue.write { db in
            try Track.filter(Column("id") == trackId).deleteAll(db)
        }
        Logger.info("Deleted track ID: \(trackId)")
    }
    ...
    func toggleFavorite(for trackId: Int64) async throws {
        try await dbQueue.write { db in
            if let track = try Track.filter(Track.Columns.trackId == trackId).fetchOne(db) {
                var mutable = track
                mutable.isFavorite.toggle()
                try mutable.update(db, columns: [Track.Columns.isFavorite])
            }
        }
    }
    ...
    // MARK: - Album/Artist Tracks
    
    func getTracks(for album: Album) async throws -> [Track] {
        guard let albumId = album.id else { return [] }
        return try await dbQueue.read { db in
            try Track
                .filter(Track.Columns.albumId == albumId)
                .order(Track.Columns.trackNumber)
                .fetchAll(db)
        }
    }
    
    func getTracks(for artist: Artist) async throws -> [Track] {
    ...
    func getArtwork(for track: Track) async -> Data? {
        // 1. Check track's own data
        if let data = track.artworkData { return data }
        
        // 2./3. Check normalized entities
        do {
            return try await dbQueue.read { db in
                // Check Album
                if let albumId = track.albumId,
                   let album = try Album.fetchOne(db, id: albumId),
                   let data = album.artworkData {
                    return data
                }
                
                // Check Artist (fallback)
                if let artistId = track.artistId,
                   let artist = try Artist.fetchOne(db, id: artistId),
                   let data = artist.artworkData {
                    return data
                }
    ...
    func getLyrics(for track: Track) async -> String? {
        do {
            return try await dbQueue.read { db in
                if let trackId = track.trackId {
                     if let lyrics = try TrackLyrics
    ...
    func saveLyrics(for trackId: Int64, lyrics: String) async {
        do {
            try await dbQueue.write { db in
                // Check if exists
                if var existing = try TrackLyrics
                    .filter(TrackLyrics.Columns.trackId == trackId)
                    .fetchOne(db) {
                    existing.lrcContent = lyrics
                    existing.dateModified = Date()
                    try existing.update(db)
                } else {
                    var newLyrics = TrackLyrics(trackId: trackId, lrcContent: lyrics)
                    try newLyrics.insert(db)
                }
            }
        } catch {
