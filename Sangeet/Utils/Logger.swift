...
enum SangeetLogLevel: Int, Comparable {
    case debug = 0
    case info = 1
    case warning = 2
    case error = 3
    ...
    static func < (lhs: SangeetLogLevel, rhs: SangeetLogLevel) -> Bool {
...
final class Logger {
    ...
    // Configuration
    private(set) var minimumLogLevel: SangeetLogLevel = .info
    private let enableConsoleLogging = true
    private let enableFileLogging = true
    
    private init() {
        let bundleIdentifier = Bundle.main.bundleIdentifier ?? "com.yashvardhan.Sangeet"
    ...
    static func debug(
        ...
        shared.log(level: .debug, message: message, file: file, line: line, function: function)
    }
    ...
    static func setMinimumLogLevel(_ level: SangeetLogLevel) {
    ...
    static func installCrashHandler() {
        ...
        for sig in signals {
            Darwin.signal(sig) { signum in
                ...
                case 11: signalName = "SIGSEGV" // Common
                default: signalName = "Unknown(\(signum))"
                }
...
private final class LogFileManager {
    ...
    private let logFileName: String = {
        let bundleID = Bundle.main.bundleIdentifier ?? "com.yashvardhan.Sangeet"
        return bundleID.hasSuffix(".debug") ? "Sangeet-debug.log" : "Sangeet.log"
    }()
